...
**Instrucciones para el profesor:** elige a dos integrantes del equipo (llámalos Alumno A y Alumno B). Cada uno corre sus comandos **en su propia computadora** — ninguno de los dos ejecuta los comandos del otro.

1. Ambos parten del mismo punto, pero cada uno crea **solo su propia rama** (nunca las dos):

**Alumno A, en su computadora:**
```bash
git checkout main
git pull
git checkout -b conflicto-a
```

**Alumno B, en su computadora (por separado):**
```bash
git checkout main
git pull
git checkout -b conflicto-b
```

2. **Ambos** editan la misma línea exacta: la etiqueta `<p class="tagline">` en `index.html`, cambiando el texto del ciclo escolar, pero cada uno pone un texto distinto — cada uno solo en su propia rama, sin ver lo que el otro escribió.

3. **Cada uno**, desde su propia rama, hace commit y push:
```bash
git add index.html
git commit -m "Actualiza texto del ciclo escolar"
git push origin conflicto-a   # o conflicto-b, según corresponda
```
