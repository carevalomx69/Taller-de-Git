...
2. Ese mismo integrante agrega un archivo `.gitattributes` para evitar problemas de finales de línea entre compañeros que usen Windows y compañeros que usen Mac (ver la nota de la Sesión 3 si no recuerdas de qué se trata):
...
3. Agrega a **todos** los demás como colaboradores: `Settings` → `Collaborators` → `Add people`.
4. **Cada uno de los demás integrantes** clona el repositorio a su propia computadora:
...
5. Antes de empezar tu tarea, siempre corre primero: `git pull`
